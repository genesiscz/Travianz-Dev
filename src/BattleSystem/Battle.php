...
class Battle 
{
        ...
        * 
        * @var double The percentage of working catapults: 1 = 100%, 0 = 0%
        */
        ...
        public function __construct(Database $database, Form $form = null)
        {           
		...
		public function procSim($post) {			
			...
			if(isset($post['a1_v']) && (isset($post['a2_v1']) || isset($post['a2_v2']) || isset($post['a2_v3']) || isset($post['a2_v4']))){
				...
				if($sum > 0){
					...
					$this->form->valuearray = $post;
				}
			}
		}
		
		private function getBattleHero($uid) {
			$heroarray = $this->database->getHero($uid);
    ...
    function calculateBattle($Attacker, $Defender, $def_wall, $att_tribe, $def_tribe, $residence, $attpop, $defpop, $type, $def_ab, $att_ab1, $att_ab2, $att_ab3, $att_ab4, $att_ab5, $att_ab6, $att_ab7, $att_ab8, $tblevel, $stonemason, $walllevel, $offhero, $hero_strenght, $deffhero, $AttackerID, $DefenderID, $AttackerWref, $DefenderWref, $conqureby, $defReinforcements = null) {
        ...
        $attacker_artefact = $this->database->getArtifactsValueInfluence($AttackerID, $AttackerWref, 3, 1, false);
        $defender_artefact = $this->database->getArtifactsValueInfluence($DefenderID, $DefenderWref, 3, 1, false);
        $strongerbuildings = $this->database->getArtifactsValueInfluence($DefenderID, $DefenderWref, 1, 1, false);
        $isWWVillage = $this->database->getVillageField($DefenderWref, 'natar');
        ...
        $DefendersAll = (!is_null($defReinforcements) ? $this->database->getEnforceVillage($DefenderWref, 0) : $defReinforcements);
        ...
        if(!empty($DefendersAll) && $DefenderWref > 0){
            ...
            foreach($DefendersAll as $defenders) {
                ...
                $userdataCache[$fromvillage] = $this->database->getUserArray($this->database->getVillageField($fromvillage, "owner"), 1);

                $enforcetribe = $userdataCache[$fromvillage]["tribe"];
                $ud=($enforcetribe - 1) * 10;
                if($defenders['from'] > 0) { //don't check nature tribe
                    $armory = $this->database->getABTech($defenders['from']); // Armory level every village enforcement
                ...
                $reinfowner = $this->database->getVillageField($fromvillage, "owner");
        ...
        if (isset($units['Att_unit']['hero']) && $units['Att_unit']['hero'] >0){
            ...
            $_result = mysqli_query($this->database->dblink,"select heroid, health from " . TB_PREFIX . "hero where `dead`='0' and `heroid`=".(int) $atkhero['heroid']);
            ...
            if ($hero_health <= $damage_health || $damage_health > 90){
                ...
                mysqli_query($this->database->dblink,"update " . TB_PREFIX . "hero set `dead` = 1, `health` = 0 where `heroid`=".(int) $hero_id);
            }else{
                mysqli_query($this->database->dblink,"update " . TB_PREFIX . "hero set `health`=`health`-".(int) $damage_health." where `heroid`=".(int) $hero_id);
            }
        }
        ...
        if (isset($units['Def_unit']['hero']) && $units['Def_unit']['hero'] >0){
            ...
            $_result = mysqli_query($this->database->dblink,"select heroid, health from " . TB_PREFIX . "hero where `dead`='0' and `heroid`=".(int) $defenderhero['heroid']);
            $fdb = mysqli_fetch_array($_result);
            $hero_id = (int) $fdb['heroid'];
            $hero_health = $fdb['health'];
        ...
        if(!empty($DefendersAll)){
            ...
            foreach($DefendersAll as $defenders) {
                if($defenders['hero'] > 0) {
                    $battleHeroesCache[$defenders['from']] = $this->getBattleHero($this->database->getVillageField($defenders['from'],"owner"));
                    $heroarraydefender = $battleHeroesCache[$defenders['from']];
                    $_result = mysqli_query($this->database->dblink,"select heroid, health from " . TB_PREFIX . "hero where `dead`='0' and `heroid`=".(int) $heroarraydefender['heroid']);
                    $fdb = mysqli_fetch_array($_result);
                    $hero_id = (int) $fdb['heroid'];
                    $hero_health = $fdb['health'];
                    $damage_health = round(100 * $result[2]);
