...
class Automation
{
    ...
    {        
        ...
        $this->database = new Database(SQL_SERVER, SQL_USER, SQL_PASS, SQL_DB);
        $this->artifacts = new Artifacts($this->database);
        $this->ranking = new Ranking($this->database);
        $this->battle = new Battle($this->database);
        ...
        $methodsArrays = [
            ...
            "spawnNatars",
            "spawnWWVillages",
            "spawnWWBuildingPlans",
            "activateArtifacts"
        ];
        
        foreach ($methodsArrays as $method) {
            $file = fopen("../../var/prevention/" . $method . ".txt", "w");
    ...
    {
        $vid = (int) $vid;
        $fdata = $this->database->getResourceLevel($vid, $use_cache);
        $popTot = 0;
        ...
        mysqli_query($this->database->dblink, $q);
    ...
    {
        $array = [];
        $array = $this->database->getProfileVillages(0, 6);
        if (!empty($array)) {
    ...
    private function clearDeleting()
    {
        ...
        if (count($needDelete) > 0) {
            ...
            foreach ($needDelete as $need) {
                ...
                $needVillages = $this->database->getVillagesID($need['uid']);
                
                // Delete all villages
                $this->database->DelVillage($needVillages);
    ...
    {
        $this->database->updateVSumField('cp');
    }

    private function buildComplete()
    {
        global $bid18, $bid10, $bid11, $bid38, $bid39;
        ...
        $res = $this->database->query_return("SELECT
        ...
        $this->database->getProfileVillages($vilIDs, 5);
        $this->database->getEnforceVillage($vilIDs, 0);
        
        // complete buildings
        foreach ($res as $indi) {
            // store village ID for later for statistical updates
            $villageData = $this->database->getVillageFields($indi['wid'], 'owner, maxcrop, maxstore, starv, pop');
            ...
            if ($this->database->query($q)) {
                ...
                if (in_array($indi['type'], [
                    10,
                    11,
                    38,
                    39
                ])) {
                    $fieldDbName = (in_array($indi['type'], [
                        10,
                        38
                    ]) ? 'maxstore' : 'maxcrop');
                ...
                if ($indi['type'] == 18)
                    Database::updateMax($villageOwner);
                ...
                if ($indi['type'] == 40 && $indi['level'] == 100) {
                    mysqli_query($this->database->dblink, "TRUNCATE " . TB_PREFIX . "bdata");
                }
                
                // TODO: find out what exactly these conditions are for
                // no special military conditioning for Teutons and Gauls
                if ($this->database->getUserField($villageOwner, "tribe", 0) != 1) {
            ...
            $this->database->addStarvationData($indi['wid']);
            
            // update the requested fields, all at once
            $this->database->setVillageFields($indi['wid'], array_keys($fieldsToSet), array_values($fieldsToSet));
        }
        
        // update statistical data for affected villages
        foreach ($villagesAffected as $affected_id)
            $this->recountPop($affected_id, false);
        ...
        if (count($dbIdsToDelete)) {
            $this->database->query("DELETE FROM " . TB_PREFIX . "bdata WHERE id IN(" . implode(',', $dbIdsToDelete) . ")");
        }
    }
    ...
    private function startNatarAttack($level, $vid, $time)
    {
        ...
        $query = mysqli_query($this->database->dblink, 'SELECT `wref` FROM `' . TB_PREFIX . 'vdata` WHERE `owner` = 3 and `capital` = 1 LIMIT 1') or die(mysqli_error($this->database->dblink));
        ...
        // -.-
        $vid = (int) $vid;
        mysqli_query($this->database->dblink, 'INSERT INTO `' . TB_PREFIX . 'ww_attacks` (`vid`, `attack_time`) VALUES (' . $vid . ', ' . $endtime . ')');
        mysqli_query($this->database->dblink, 'INSERT INTO `' . TB_PREFIX . 'ww_attacks` (`vid`, `attack_time`) VALUES (' . $vid . ', ' . ($endtime + 1) . ')');
        ...
        // wave 2
        $ref2 = $this->database->addAttack($row['wref'], 0, $units[1][0], $units[1][1], 0, $units[1][2], $units[1][3], $units[1][4], $units[1][5], 0, 0, 0, 3, 40, 0, 0, 0, 20, 20, 0, 20, 20, 20, 20, [
            'vid' => $vid,
            'endtime' => ($endtime + 1)
        ]);
    ...
    {
        $query = mysqli_query($this->database->dblink, 'SELECT vid, attack_time FROM `' . TB_PREFIX . 'ww_attacks` WHERE `attack_time` <= ' . time());
        while ($row = mysqli_fetch_assoc($query)) {
            // delete the attack
            $query3 = mysqli_query($this->database->dblink, 'DELETE FROM `' . TB_PREFIX . 'ww_attacks` WHERE `vid` = ' . (int) $row['vid'] . ' AND `attack_time` = ' . (int) $row['attack_time']);
        }
    }
    ...
    private function TradeRoute()
    {
        ...
        $vilIDs = array_keys($vilIDs);
        $this->database->getVillageByWorldID($vilIDs);
    ...
    {        
        ...
        foreach ($dataarray as $data) {
            $userData_from = $this->database->getUserFields($this->database->getVillageField($data['from'], "owner"), "alliance, tribe", 0);
            $userData_to = $this->database->getUserFields($this->database->getVillageField($data['to'], "owner"), "alliance, tribe", 0);
            ...
            $endtime = $this->database->getWalkingTroopsTime($data['from'], $data['to'], 0, 0, [
                $targettribe
            ], 0) + $data['endtime'];
        ...
        foreach ($dataarray1 as $data1) {
            ...
            if ($data1['send'] > 1) {
                $targettribe1 = $this->database->getUserFields($this->database->getVillageField($data1['to'], "owner"), "alliance, tribe", 0)['tribe'];
                $send = $data1['send'] - 1;
    ...
    private function sendResource2($wtrans, $ctrans, $itrans, $crtrans, $from, $to, $tribe, $send)
    {
        ...
        if ($availableWood + $availableClay + $availableIron + $availableCrop > 0) {
            if ($availableWood < $wtrans)
                $wtrans = $availableWood;
            if ($availableClay < $ctrans)
                $ctrans = $availableClay;
            if ($availableIron < $itrans)
                $itrans = $availableIron;
            if ($availableCrop < $crtrans)
                $crtrans = $availableCrop;
            
            $merchant2 = ($this->getTypeLevel(17, $from) > 0) ? $this->getTypeLevel(17, $from) : 0;
            $used2 = $this->database->totalMerchantUsed($from, false);
            ...
            if ($merchantAvail2 > 0 && $reqMerc <= $merchantAvail2) {
                if ($this->database->getVillageState($to)) {
                    $timetaken = $this->database->getWalkingTroopsTime($from, $to, 0, 0, [
                        $tribe
                    ], 0);
    ...
    private function resolveCatapultsDestruction(&$bdo, &$battlepart, &$info_cat, &$data, $catapultTarget, $twoRowsCatapultSetup, $isSecondRow, $catp_pic, $can_destroy, $isoasis, &$village_destroyed, $tribe)
    {
        ...
        if (isset($catapultTarget)) {
            ...
            $catapultsDamage = $this->battle->calculateCatapultsDamage($data['t8'], $battlepart['catapults']['upgrades'], $battlepart['catapults']['durability'], $battlepart['catapults']['attackDefenseRatio'], $battlepart['catapults']['strongerBuildings'], $battlepart['catapults']['moraleBonus']);
            
            $newLevel = $this->battle->calculateNewBuildingLevel($tblevel, $catapultsDamage / ($twoRowsCatapultSetup ? 2 : 1));
            
            // If that building was present in the building queue, we have to modify his level or remove it
            $this->database->modifyBData($data['to'], $tbid, [
                $newLevel,
                $tblevel
            ], $tribe);
            ...
            if ($newLevel == 0) {
                ...
                $this->database->setVillageLevel($data['to'], $fieldsToSet, $fieldValuesToSet);
                
                $buildarray = $GLOBALS["bid" . $tbgid];
                
                // (great) warehouse level was changed
                if ($tbgid == 10 || $tbgid == 38) {
                    $this->database->setMaxStoreForVillage($data['to'], $buildarray[$newLevel]['attri']);
                }
                
                // (great) granary level was changed
                if ($tbgid == 11 || $tbgid == 39) {
                    $this->database->setMaxCropForVillage($data['to'], $buildarray[$newLevel]['attri']);
                }
                
                // oasis cannot be destroyed
                $pop = $this->recountPop($data['to'], false);
                if ($isoasis == 0 && $pop == 0 && $can_destroy == 1)
                    $village_destroyed = 1;
                ...
                if ($isSecondRow) {
                    ...
                    if ($tbgid == 18) {
                        $info_cat .= $this->database->checkEmbassiesAfterBattle($data['to'], $bdo['f' . $catapultTarget], false);
                    }
    ...
    private function sendunitsComplete()
    {
        ...
        if ($dataarray && count($dataarray)) {
            ...
            $vilIDs = array_keys($vilIDs);
            $this->database->getProfileVillages($vilIDs, 5);
            $this->database->getUnit($vilIDs);
            $this->database->getEnforceVillage($vilIDs, 0);
            $this->database->getMovement(34, $vilIDs, 1);
            $this->database->getABTech($vilIDs);
            ...
            foreach ($dataarray as $data) {
                ...
                $Attacker['id'] = $this->database->getUserArray($this->database->getVillageField($data['from'], "owner"), 1)["id"];
                $AttackerID = $Attacker['id'];
                $owntribe = $this->database->getUserArray($this->database->getVillageField($data['from'], "owner"), 1)["tribe"];
                $ownally = $this->database->getUserArray($this->database->getVillageField($data['from'], "owner"), 1)["alliance"];
                $from = $this->database->getMInfo($data['from']);
                $fromF = $this->database->getVillage($data['from']);
                
                // It's a village
                if ($isoasis == 0) {
                    $DefenderUserData = $this->database->getUserArray($this->database->getVillageField($data['to'], "owner"), 1);
                    ...
                    if ($evasion == 1 && $maxevasion > 0 && $gold > 1 && !$cannotsend && $dataarray[$data_num]['attack_type'] > 2) {
                        ...
                        if ($totaltroops > 0) {
                            ...
                            $this->database->updateUserField($DefenderID, [
                                "gold",
                                "maxevasion"
                            ], [
                                $newgold,
                        ...
                        // modify units in DB
                        $this->database->modifyUnit($data['to'], $evasionUnitModifications_units, $evasionUnitModifications_amounts, $evasionUnitModifications_modes);
                    }
                    
                    // get defence units
                    $enforDefender = [];
                    $rom = $ger = $gal = $nat = $natar = 0;
                    $Defender = $this->database->getUnit($data['to'], false);
                    ...
                    $def_wall = $this->database->getFieldLevel($data['to'], 40, false);
                    $att_tribe = $owntribe;
                    ...
                    $residence = $this->database->getFieldLevelInVillage($data['to'], '25, 26', false);
                    ...
                    if (($data['t7']) > 0 && $type == 3) {
                        $basearraywall = $to;
                        if (($walllevel = $this->database->getFieldLevel($basearraywall['wref'], 40, false)) > 0) {
                    ...
                    $tblevel = 1;
                    $stonemason = $this->database->getFieldLevelInVillage($data['to'], 34);
                } else { // It's an oasis
                    
                    $DefenderUserData = $this->database->getUserArray($this->database->getOasisField($data['to'], "owner"), 1);
                    $Defender['id'] = $DefenderUserData["id"];
                    $DefenderID = $Defender['id'];
                    $targettribe = $DefenderUserData["tribe"];
                    $targetally = $DefenderUserData["alliance"];
                ...
                $varray = $this->database->getProfileVillages($to['owner'], 0, false);
                
                if ($to['owner'] == $from['owner'])
                    $varray1 = $varray;
                else
                    $varray1 = $this->database->getProfileVillages($from['owner'], 0, false);
                ...
                if (PEACE == 0 || $targettribe == 4 || $targettribe == 5 || $scout) {
                    ...
                    if (empty($scout) || $NatarCapital) {
                        ...
                        $this->database->modifyUnit($data['to'], [
                            "99o"
                        ], [
                            $totaltraped_att
                        ], [
                        ...
                        if ($totaltraped_att > 0) {
                            $prisoners2 = $this->database->getPrisoners2($data['to'], $data['from'], false);
                            if (empty($prisoners2)) {
                    ...
                    if (isset($Attacker['uhero']) && $Attacker['uhero'] > 0) {
                        $AttackerHeroID = $this->database->getHeroField($from['owner'], "heroid");
                    }
                    ...
                    if (isset($Defender['hero']) && $Defender['hero'] > 0) {
                        $DefendersHeroID[] = $this->database->getHeroField($DefenderID, "heroid");
                    ...
                    if (count($enforcementarray) > 0) {
                        foreach ($enforcementarray as $enforcement) {
                            if ($enforcement['hero'] > 0) {
                                $heroOwner = $this->database->getVillageField($enforcement['from'], "owner");
                                $DefendersHeroID[] = $this->database->getHeroField($heroOwner, "heroid");
                            }
                        }
                    }
                    ...
                    $battlepart = $this->battle->calculateBattle($Attacker, $Defender, $def_wall, $att_tribe, $def_tribe, $residence, $attpop, $defpop, $type, $def_ab, $att_ab1, $att_ab2, $att_ab3, $att_ab4, $att_ab5, $att_ab6, $att_ab7, $att_ab8, $tblevel, $stonemason, $walllevel, 0, 0, 0, $AttackerID, $DefenderID, $AttackerWref, $DefenderWref, $conqureby, $enforcementarray);
                    ...
                    if (count($varray) > 1 && !$this->database->villageHasArtefact($DefenderWref)) {
                        $can_destroy = 1;
                    } else
                        $can_destroy = 0;
                    ...
                    if ($isoasis == 0) {
                        if ($type == 3) {
                            if (($data['t7'] - $traped7) > 0) {
                                if ($walllevel > 0) {
                                    $ramsDamage = $this->battle->calculateCatapultsDamage($data['t7'], $battlepart['rams']['upgrades'], $battlepart['rams']['durability'], $battlepart['rams']['attackDefenseRatio'], $battlepart['rams']['strongerBuildings'], $battlepart['rams']['moraleBonus']);
                                    $newLevel = $this->battle->calculateNewBuildingLevel($walllevel, $ramsDamage);
                                    
                                    if ($newLevel == 0) {
                                        $info_ram = "" . $ram_pic . ",Wall <b>destroyed</b>.";
                                        $this->database->setVillageLevel($data['to'], [
                                            "f" . $wallid,
                                            "f" . $wallid . "t"
                                        ], [
                                            0,
                                        ...
                                        $this->database->setVillageLevel($data['to'], "f" . $wallid . "", $newLevel);
                                    }
                                    
                                    // If the wall got damaged/destroyed during the attack
                                    // we need to recalculate the whole battle
                                    if ($newLevel != $walllevel) {
                                        $battlepart = $this->battle->calculateBattle($Attacker, $Defender, $newLevel, $att_tribe, $def_tribe, $residence, $attpop, $defpop, $type, $def_ab, $att_ab1, $att_ab2, $att_ab3, $att_ab4, $att_ab5, $att_ab6, $att_ab7, $att_ab8, $tblevel, $stonemason, $newLevel, 0, 0, 0, $AttackerID, $DefenderID, $AttackerWref, $DefenderWref, $conqureby, $enforcementarray);
                                    }
                                } else
                            ...
                            if (($data['t8'] - $traped8) > 0) {
                                ...
                                if ($pop <= 0) {
                                    ...
                                    $basearray = $data['to'];
                                    $bdo = $this->database->getResourceLevel($basearray, false);
                    ...
                    if (count($enforcementarray2) > 0) {
                        foreach ($enforcementarray2 as $enforce2) {
                            ...
                            if ($enforce2['hero'] > 0) {
                                $reinfTribe = ($enforce2['from'] == 0) ? 4 : $this->database->getUserField($this->database->getVillageField($enforce2['from'], "owner"), "tribe", 0);
                    ...
                    // kill own defence
                    $unitlist = $this->database->getUnit($data['to'], false);
                    ...
                    // modify units in DB
                    $this->database->modifyUnit($data['to'], $unitModifications_units, $unitModifications_amounts, $unitModifications_modes);
                    
                    // kill other defence in village
                    // ... once again, units could have changed, so we need to reselect
                    $enforcementarray3 = $this->database->getEnforceVillage($data['to'], 0);
                    foreach ($enforcementarray3 as $enforce) {
                        $life = '';
                        $notlife = '';
                        $wrong = false;
                        if ($enforce['from'] != 0) {
                            $tribe = $this->database->getUserArray($this->database->getVillageField($enforce['from'], "owner"), 1)["tribe"];
                        } else
                            $tribe = 4;
                        ...
                        if ($enforce['hero'] > 0) {
                            ...
                            $reinfTribe = ($enforce['from'] == 0) ? 4 : $this->database->getUserField($this->database->getVillageField($enforce['from'], "owner"), "tribe", 0);
                        ...
                        foreach ($enforceModificationsById as $enforceId => $enforceArray) {
                            $this->database->modifyEnforce($enforceId, $enforceArray['units'], $enforceArray['amounts'], $enforceArray['modes']);
                        }
                    ...
                    $this->database->modifyAttack3($data['ref'], implode(', ', $p_units));
                    ...
                    if ($Attacker['uhero'] > 0) {
                        ...
                        $this->database->modifyHeroXp("experience", $heroxp, $AttackerHeroID);
                    }
                    ...
                    if ($Defender['hero'] > 0) {
                        ...
                        foreach ($DefendersHeroID as $HeroID) {
                            $this->database->modifyHeroXp("experience", $defheroxp, $HeroID);
                        }
                    }
                    ...
                    $this->database->modifyPoints($toF['owner'], [
                        'dpall',
                        'dp'
                    ], [
                        $totalpoint_def,
                    ...
                    $this->database->modifyPoints($from['owner'], [
                        'apall',
                        'ap'
                    ], [
                        $totalpoint_att,
                    ...
                    $this->database->modifyPointsAlly($targetally, [
                        'Adp',
                        'dp'
                    ], [
                        $totalpoint_def,
                    ...
                    $this->database->modifyPointsAlly($ownally, [
                        'Aap',
                        'ap'
                    ], [
                        $totalpoint_att,
                    ...
                    if ($isoasis == 0) {
                        ...
                        $crannySpy = $this->database->getArtifactsValueInfluence($to_owner, $data['to'], 7, $cranny * $def_bonus);
                        ...
                        $villageData = $this->database->getVillageFields($data['to'], 'clay, iron, wood, crop', false);
                        ...
                        if ($conqureby > 0) { // 10% from owner proc village owner - fix by ronix - exploit fixed by iopietro
                            ...
                            $villageData = $this->database->getVillageFields($conqureby, 'clay, iron, wood, crop', false);
                            ...
                            $oasisData = $this->database->getOasisFields($data['to'], false);
                    ...
                    for ($i = 0; $i < 5; $i++) {
                        ...
                        if (empty($avtotal) || ($btotal < 1 && $bmod < 1))
                            break;
                    ...
                    if (($data['t9'] - $dead9 - $traped9) > 0 && $isoasis == 0) {
                        if ($type == 3) {
                            ...
                            $expArray = $this->database->getVillageFields($from['wref'], 'exp1, exp2, exp3');
                            $exp1 = $expArray['exp1'];
                            $exp2 = $expArray['exp2'];
                            $exp3 = $expArray['exp3'];
                            ...
                            $user_cps = $this->database->getUserArray($from['owner'], 1)['cp'];
                            ...
                            if ($user_cps >= $need_cps) {
                                if (count($varray) > 1 && $to['capital'] != 1 && $villexp < $canconquer) {
                                    if ($to['owner'] != 3 || $to['name'] != 'WW Buildingplan') {
                                        ...
                                        if ($this->database->getFieldLevelInVillage($data['to'], '25, 26')) {
                                            $nochiefing = 1;
                                        ...
                                        if (!isset($nochiefing)) {
                                            ...
                                            for ($i = 0; $i < ($data['t9'] - $dead9 - $traped9); $i++) {
                                                ...
                                                if ($owntribe == 1)
                                                    $reducedLoyalty = rand(20, 30);
                                                else
                                                    $reducedLoyalty = rand(20, 25);
                                                
                                                // If a Big Party is active in the attacking village
                                                if ($from['celebration'] > $time && $from['type'] == 2)
                                                    $reducedLoyalty += 5;
                                                
                                                // If a Big Party is active in the target village
                                                if ($to['celebration'] > $time && $to['type'] == 2)
                                                    $reducedLoyalty -= 5;
                                                ...
                                                if ($owntribe == 2 && $this->getTypeLevel(35, $data['from']) > 0)
                                                    $reducedLoyalty /= 2;
                                            ...
                                            if (($toF['loyalty'] - $reducedLoyaltyTotal) > 0) {
                                                ...
                                                $this->database->setVillageField($data['to'], 'loyalty', ($toF['loyalty'] - $reducedLoyaltyTotal));
                                            } else if (!$village_destroyed) {
                                                // you took over the village
                                                $villname = addslashes($this->database->getVillageField($data['to'], "name"));
                                                $artifact = reset($this->database->getOwnArtefactInfo($data['to']));
                                                ...
                                                $this->database->setVillageFields($data['to'], [
                                                    'loyalty',
                                                    'owner'
                                                ], [
                                                    0,
                                                    $this->database->getVillageField($data['from'], "owner")
                                                ]);
                                                ...
                                                $poparray = $this->database->getVSumField([
                                                    $AttackerID,
                                                    $DefenderID
                                                ], "pop");
                                                ...
                                                $this->database->clearExpansionSlot($data['to'], 1);
                                                
                                                $expArray = $this->database->getVillageFields($data['from'], 'exp1, exp2, exp3');
                                                $exp1 = $expArray['exp1'];
                                                $exp2 = $expArray['exp2'];
                                                $exp3 = $expArray['exp3'];
                                                ...
                                                $this->database->deleteTradeRoutesByVillage($data['to']);
                                                
                                                // Update data in the database
                                                $this->database->setVillageLevel($data['to'], $newLevels_fieldNames, $newLevels_fieldValues);
                                                
                                                // Recount the population
                                                $pop = $this->recountPop($data['to'], false);
                                                
                                                // Kill and reassign the hero to the capital village, if registered in the conquered one
                                                $this->database->reassignHero($data['to']);
                                            }
                                        }
                                    } else
                    ...
                    if (($data['t11'] - $dead11 - $traped11) > 0) { // hero
                        ...
                        if ($isoasis != 0) { // oasis fix by ronix
                            if ($to['owner'] != $from['owner']) {
                                $troopcount = $this->database->countOasisTroops($data['to'], false);
                                $canqured = $this->database->canConquerOasis($data['from'], $data['to'], false);
                            ...
                            if (!empty($artifact)) {
                                if ($type == 3) {
                                    if (empty($artifactError = $this->database->canClaimArtifact($data['from'], $artifact['vref'], $artifact['size'], $artifact['type']))) {
	...
	";
                            ...
                            if ($isoasis == 0) {
                                $walllevel = $this->database->getFieldLevelInVillage($data['to'], '31, 32, 33');
                                $residencelevel = $this->database->getFieldLevelInVillage($data['to'], 25);
                                $palacelevel = $this->database->getFieldLevelInVillage($data['to'], 26);
                                ...
                                $info_spy = "" . $spy_pic . ",";
                                if ($residencelevel > 0)
                                    $info_spy .= $residenceimg . " Residence level:<b>" . $residencelevel . "</b><br />";
                                elseif ($palacelevel > 0)
                                    $info_spy .= $palaceimg . " Palace level: <b>" . $palacelevel . "</b><br />";
                                
                                if ($walllevel > 0)
                                    $info_spy .= $wallimg . " Wall level: <b>" . $walllevel . "</b><br />";
                                $info_spy .= $crannyimg . " Total crannies capacity: <b>" . $crannySpy . "</b>";
                            } else
                                $info_spy = "" . $spy_pic . ", There are no informations to show";
                        }
                    ...
                    if (!empty($scout)) {
                        for ($i = 1; $i <= 10; $i++) {
                            if ($battlepart['casualties_attacker'][$i]) {
                                if ($from['owner'] == 3) {
                                    ...
                                    $this->database->addNotice($to['owner'], $to['wref'], $targetally, 20, '' . addslashes($from['name']) . ' scouts ' . addslashes($to['name']) . '', $data2 . ',,' . $info_troop, $AttackArrivalTime);
                                    break;
                        ...
                        if ($type == 3 && $totalsend_att - ($totaldead_att + $totaltraped_att) > 0) {
                            $prisoners = $this->database->getPrisoners([
                                $to['wref']
                            ], 0, false)[$to['wref'] . '0'];
                            if (count($prisoners) > 0) {
                                ...
                                foreach ($prisoners as $prisoner) {
                                    ...
                                    if ($prisoner['from'] == $from['wref']) {
                                        ...
                                        $this->database->modifyAttack2($data['ref'], [
                                            1,
                                            2,
                                            3,
                                            4,
                                        ...
                                        if ($p_alliance == $ownally || $friend || $neutral) {
                                            ...
                                            $troopsTime = $this->database->getWalkingTroopsTime($prisoner['from'], $prisoner['wref'], $p_owner, $p_tribe, $prisoner, 1, 't');
                                            $p_time = $this->database->getArtifactsValueInfluence($p_owner, $prisoner['from'], 2, $troopsTime);
                                ...
                                if (count($movementType)) {
                                    $this->database->addMovement($movementType, $movementFrom, $movementTo, $movementRef, $movementTime, $movementEndtime);
                                }
                                
                                $this->database->deletePrisoners($prisoners2delete);
                                ...
                                $this->database->addGeneralAttack($ownDeads + $anotherDeads);
                                ...
                                // Repair needed traps
                                if ($newtraps > 0) {
                                    $repairDuration = $this->database->getArtifactsValueInfluence($to['owner'], $to['wref'], 5, round(($bid19[max($this->getTypeLevel(36, $to['wref']), 1)]['attri'] / 100) * $u99['time'] / SPEED));
                                    $this->database->addUnitToTraining($to['wref'], 99, $newtraps, $u99['pop'], $repairDuration, 0);
                                }
                    ...
                    if ($totalsend_att - ($totaldead_att + (isset($totaltraped_att) ? $totaltraped_att : 0)) > 0) {
                        $returningTroops = [];
                        for ($i = 1; $i <= 11; $i++)
                            $returningTroops['t' . $i] = $data['t' . $i] - ${'traped' . $i} - ${'dead' . $i};
                        $troopsTime = $this->database->getWalkingTroopsTime($from['wref'], $to['wref'], $from['owner'], $owntribe, $returningTroops, 1, 't');
                        $endtime = $this->database->getArtifactsValueInfluence($from['owner'], $from['wref'], 2, $troopsTime);
                        ...
                        if ($chiefing_village != 1) {
                            $this->database->addMovement(4, $DefenderWref, $AttackerWref, $data['ref'], $AttackArrivalTime, $endtime, 1, $steal[0], $steal[1], $steal[2], $steal[3]);
                            if ($type !== 1) {
                                if ($isoasis == 0)
                                    ...
                                    if ($conqureby > 0)
                                        $this->database->modifyResource($conqureby, $steal[0], $steal[1], $steal[2], $steal[3], 0);
                                    else
                    ...
                    if ($village_destroyed == 1 && $can_destroy == 1) {
                        if ($to['capital'] == 1) {
                            ...
                            // Set the new capital
                            $this->database->changeCapital($mostPopulatedVillage['wref']);
                        }
                        
                        // Delete the village
                        $this->database->DelVillage($data['to']);
                    ...
                    $troopsTime = $this->database->getWalkingTroopsTime($from['wref'], $to['wref'], $from['owner'], $owntribe, $data, 1, 't');
                    $endtime = $this->database->getArtifactsValueInfluence($from['owner'], $from['wref'], 2, $troopsTime);
                    $endtime += $AttackArrivalTime;
                    ...
                    $this->database->addNotice($to['owner'], $to['wref'], $targetally, 23, '' . addslashes($from['name']) . ' attacks ' . addslashes($to['name']) . '', $data2, $time);
                }
                
                // Update starvation data
                $this->database->addStarvationData($to['wref']);
    ...
    private function sendreinfunitsComplete()
    {
        ...
        if ($dataarray && count($dataarray)) {
            ...
            $this->database->getProfileVillages($vilIDs, 5);
            $this->database->getUnit($vilIDs);
            $this->database->getEnforce(array_keys($tos), array_keys($froms));
            $this->database->getVillageByWorldID($vilIDs);
            ...
            foreach ($dataarray as $data) {
                ...
                if ($data['from'] == 0) {
                    ...
                    $this->database->addNotice($to['owner'], $to['wref'], (isset($targetally) ? $targetally : 0), 8, 'village of the elders reinforcement ' . addslashes($to['name']), $data_fail, $AttackArrivalTime);
                    ...
                    if ($data['t11'] > 0 && !$isoasis && !$troopsPresent) {
                        ...
                        if ($AttackerID == $DefenderID) {
                            ...
                            if ($this->getTypeLevel(37, $data['to']) > 0) {
                                ...
                                $heroid = $this->database->getHeroField($DefenderID, 'heroid');
                                $this->database->modifyHero("wref", $data['to'], $heroid);
                                $HeroTransfer = 1;
                            }
                        }
                    }
                    ...
                    $this->database->addNotice($from['owner'], $from['wref'], (isset($ownally) ? $ownally : 0), 8, '' . addslashes($from['name']) . ' reinforcement ' . addslashes($to_name) . '', $data_fail, (isset($AttackArrivalTime) ? $AttackArrivalTime : time()));
                    if ($from['owner'] != $to['owner']) {
                        $this->database->addNotice($to['owner'], $to['wref'], (isset($targetally) ? $targetally : 0), 8, '' . addslashes($from['name']) . ' reinforcement ' . addslashes($to_name) . '', $data_fail, (isset($AttackArrivalTime) ? $AttackArrivalTime : time()));
                    }
                    // update status
                    $movementProcIDs[] = $data['moveid'];
                }
                ...
                // Update starvation data
                $this->database->addStarvationData($data['to']);
            ...
            $this->database->setMovementProc(implode(', ', $movementProcIDs));
        }
    }
    ...
    {        
        ...
        if ($dataarray && count($dataarray)) {
            ...
            $this->database->getProfileVillages(array_keys($vilIDs), 5);
            $this->database->getOasisEnforce($vilIDs, 0);
            $this->database->getOasisEnforce($vilIDs, 1);
        ...
        if ($dataarray && count($dataarray)) {
            foreach ($dataarray as $data) {
                ...
                $this->database->modifyResource($data['to'], 750, 750, 750, 750, 1);
                $movementProcIDs[] = $data['moveid'];
            }
            $this->database->setMovementProc(implode(', ', $movementProcIDs));
        }
    ...
    private function sendSettlersComplete()
    {
        ...
        $this->database->addMovement($types, $froms, $tos, $refs, $times, $endtimes);
        $this->database->setMovementProc(implode(', ', $movementProcIDs));
        $this->database->setFieldTaken($fieldIDs);
        $this->database->addUnits($addUnitsWrefs);
        $this->database->addTech($addTechWrefs);
        $this->database->addABTech($addABTechWrefs);
    }
    ...
    private function spawnNatars()
    {
        ...
        if ($this->database->areArtifactsSpawned() || strtotime(START_DATE) + (NATARS_SPAWN_TIME * 86400) > time())
            return;
        ...
        $this->database->displaySystemMessage(ARTEFACT);
    }
    ...
    private function spawnWWVillages()
    {
        ...
        if (!$this->database->areArtifactsSpawned() || $this->database->areWWVillagesSpawned() || strtotime(START_DATE) + (NATARS_WW_SPAWN_TIME * 86400) > time())
            return;
        ...
        // Write the system message
        $this->database->displaySystemMessage(WWVILLAGEMSG);
    }
    ...
    private function spawnWWBuildingPlans()
    {
        ...
        // and if it's the time to spawn them or not
        if (!$this->database->areArtifactsSpawned() || $this->database->areArtifactsSpawned(true) || strtotime(START_DATE) + (NATARS_WW_BUILDING_PLAN_SPAWN_TIME * 86400) > time())
            return;
        ...
        $this->database->displaySystemMessage(PLAN_INFO);
    }
    ...
    private function activateArtifacts()
    {
        ...
        if (!$this->database->areArtifactsSpawned())
            return;
    ...
    {
        $oasisInfoArray = $this->database->getOasisV($bountywid);
    ...
    {
        // Get village infos
        $villageInfoArray = $this->database->getVillage($bountywid);
        
        // Get building and resource fields array
        $resArray = $this->database->getResourceLevel($bountywid, false);
        
        // Get oasis array
        $oasisArray = $this->database->getOasis($bountywid);
        ...
        $upkeep = $this->database->getUpkeep($this->getAllUnits($bountywid), 0, $bountywid);
    ...
    {
        $ownunit = $this->database->getUnit($base, $use_cache);
        ...
        $enforceoasis = $this->database->getOasisEnforce($base, 0, $use_cache);
        ...
        $enforceoasis1 = $this->database->getOasisEnforce($base, 1, $use_cache);
    ...
    private function bountyGetWoodProd($resArray, $oasisNumber)
    {
        ...
        for ($i = 1; $i <= 38; $i++) {
            ...
            if ($resArray['f' . $i . 't'] == 5)
                $sawmill = $resArray['f' . $i];
        }
        
        for ($i = 0; $i <= count($woodholder) - 1; $i++)
            $wood += $bid1[$resArray[$woodholder[$i]]]['prod'];
        
        if ($sawmill >= 1)
            $wood += $wood / 100 * $bid5[$sawmill]['attri'];
        if ($oasisNumber[0] > 0)
            $wood += $wood * 0.25 * $oasisNumber[0];
    ...
    private function bountyGetClayProd($resArray, $oasisNumber)
    {
        ...
        for ($i = 0; $i <= count($clayholder) - 1; $i++)
            $clay += $bid2[$resArray[$clayholder[$i]]]['prod'];
        
        if ($brick >= 1)
            $clay += $clay / 100 * $bid6[$brick]['attri'];
        if ($oasisNumber[1] > 0)
            $clay += $clay * 0.25 * $oasisNumber[1];
    ...
    private function bountyGetIronProd($resArray, $oasisNumber)
    {
        ...
        for ($i = 1; $i <= 38; $i++) {
            ...
            if ($resArray['f' . $i . 't'] == 7)
                $foundry = $resArray['f' . $i];
        }
        
        for ($i = 0; $i <= count($ironholder) - 1; $i++)
            $iron += $bid3[$resArray[$ironholder[$i]]]['prod'];
        
        if ($foundry >= 1)
            $iron += $iron / 100 * $bid7[$foundry]['attri'];
        if ($oasisNumber[2] > 0)
            $iron += $iron * 0.25 * $oasisNumber[2];
    ...
    private function bountyGetCropProd($resArray, $oasisNumber)
    {
        ...
        for ($i = 1; $i <= 38; $i++) {
            ...
            if ($resArray['f' . $i . 't'] == 8)
                $grainmill = $resArray['f' . $i];
            if ($resArray['f' . $i . 't'] == 9)
                $bakery = $resArray['f' . $i];
        }
        for ($i = 0; $i <= count($cropholder) - 1; $i++)
            $crop += $bid4[$resArray[$cropholder[$i]]]['prod'];
        ...
        if ($oasisNumber[3] > 0)
            $crop += $crop * 0.25 * $oasisNumber[3];
        
        if (!empty($resArray['vref']) && is_numeric($resArray['vref'])) {
            $who = $this->database->getVillageField($resArray['vref'], "owner");
    ...
    {
        $time = time();
        $trainlist = $this->database->getTrainings();
        if (count($trainlist) > 0) {
            ...
            $vilIDs = array_keys($vilIDs);
            $this->database->getProfileVillages($vilIDs, 5);
            $this->database->cacheResourceLevels($vilIDs);
            $this->database->getUnit($vilIDs);
            $this->database->getEnforceVillage($vilIDs, 0);
            $this->database->getMovement(3, $vilIDs, 0);
            $this->database->getMovement(4, $vilIDs, 1);
            $this->database->getMovement(5, $vilIDs, 0);
            $this->database->getOasisEnforce($vilIDs, 0);
            $this->database->getOasisEnforce($vilIDs, 1);
            $this->database->getPrisoners($vilIDs, 1);
            ...
            foreach ($trainlist as $train) {
                ...
                if ($timepast <= 0 && $train['amt'] > 0) {
                    ...
                    if ($train['unit'] > 60 && $train['unit'] != 99) {
                        $this->database->modifyUnit($train['vref'], [
                            $train['unit'] - 60
                        ], [
                            $trained
                        ], [
                    ...
                    if ($train['amt'] - $trained <= 0) {
                        $this->database->addUnitToTraining($train['id'], 0, 0, 0, 0, 1);
                    }
                }
                ...
                // Update starvation data
                $this->database->addStarvationData($train['vref']);
            }
        }
    }
    ...
    private function demolitionComplete()
    {
        ...
        foreach ($varray as $vil) {
            if ($vil['timetofinish'] <= time()) {
                ...
                if ($this->database->getVillageField($vil['vref'], 'natar') == 1 && $type == 40)
                    $clear = ""; // fix by ronix - fixed by iopietro
                ...
                $this->database->delDemolition($vil['vref'], true);
                
                if ($type == 18)
                    Database::updateMax($this->database->getVillageField($vil['vref'], 'owner'));
            }
        }
    }
    ...
    private function updateHero()
    {
        ...
        if (!empty($harray)) {
            ...
            $unitData = $this->database->getUnit($heroVillageIDs);
            ...
            foreach ($harray as $hdata) {
                ...
                if ($hdata['trainingtime'] < time() && $hdata['inrevive'] == 1) {
                    mysqli_query($this->database->dblink, "UPDATE " . TB_PREFIX . "units SET hero = 1 WHERE vref = " . (int) $hdata['wref'] . "");
                ...
                if (count($columns))
                    $this->database->modifyHero($columns, $columnValues, $hdata['heroid'], $modes);
            }
            
            if (count($lastUpdateIDs)) {
                mysqli_query($this->database->dblink, "UPDATE " . TB_PREFIX . "hero SET lastupdate = $timeNow WHERE heroid IN(" . implode(', ', $lastUpdateIDs) . ")");
            }
        }
    }
    ...
    private function updateStore()
    {
        ...
        $result = mysqli_query($this->database->dblink, 'SELECT * FROM `' . TB_PREFIX . 'fdata`');
        
        mysqli_begin_transaction($this->database->dblink);
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            mysqli_query($this->database->dblink, 'UPDATE `' . TB_PREFIX . 'vdata` SET `maxstore` = ' . (int) $ress . ', `maxcrop` = ' . (int) $crop . ' WHERE `wref` = ' . (int) $row['vref']);
        }
        mysqli_commit($this->database->dblink);
    }
    ...
    private function checkInvitedPlayes()
    {
        ...
        $this->database->getProfileVillages($userIDs);
        
        // continue...
        foreach ($array as $user) {
            $numusers = mysqli_fetch_array(mysqli_query($this->database->dblink, "SELECT Count(*) as Total FROM " . TB_PREFIX . "users WHERE id = " . (int) $user['invited']), MYSQLI_ASSOC);
            if ($numusers['Total'] > 0) {
    ...
    {
        mysqli_query($this->database->dblink, "
    ...
    private function MasterBuilder()
    {
        ...
        foreach ($array as $master) {
            ...
            if ($tribe == 1) {
                if ($master['field'] < 19) {
                    $bdata = $this->database->getDorf1Building($master['wid']);
                    $bdataTotal = count($bdata);
                    $bbdata = count($this->database->getDorf2Building($master['wid']));
                } else {
                    $bdata = $this->database->getDorf2Building($master['wid']);
                    $bdataTotal = count($bdata);
                    $bbdata = count($this->database->getDorf1Building($master['wid']));
                }
            } else {
                $bdata = array_merge($this->database->getDorf1Building($master['wid']), $this->database->getDorf2Building($master['wid']));
                $bdataTotal = $bbdata = count($bdata);
            }
            
            if ($this->database->getUserField($owner, 'plus', 0) > time() || $ww > 0) {
                if ($bbdata < 2)
                    $inbuild = 2;
                else
                    $inbuild = 1;
            ...
            if ($bdataTotal < $inbuild && $buildwood <= $villwood && $buildclay <= $villclay && $buildiron <= $villiron && $buildcrop <= $villcrop && $usergold > 0) {
                ...
                if (!empty($bdata)) {
                    foreach ($bdata as $masterLoop)
                        $time += ($masterLoop['timestamp'] - time());
                }
                ...
                $this->database->updateUserField($owner, 'gold',-- $usergold, 1);
    ...
    private function starvation()
    {
        ...
        $starvarray = $this->database->getProfileVillages(0, 7);
        foreach ($starvarray as $starv)
            $this->database->addStarvationData($starv['wref']);
        ...
        // Cache
        $this->database->getEnforceVillage($vilIDs, 0);
        $this->database->getOasisEnforce($vilIDs, 2);
        $this->database->getOasisEnforce($vilIDs, 3);
        $this->database->getPrisoners($vilIDs, 1);
        $this->database->getMovement(3, $vilIDs, 0);
        $this->database->getMovement(4, $vilIDs, 1);
        ...
        foreach ($starvarray as $starv) {
            ...
            $upkeep = $starv['pop'] + $this->database->getUpkeep($unitarrays, 0, $starv['wref']);
            
            $enforceArrays = $prisonerArrays = $unitArrays = $attackArrays = $allTroopsArray = $starvingTroops = $killedUnits = [];
            
            $enforceArrays = [
                $this->database->getOasisEnforce($starv['wref'], 2),
                $this->database->getOasisEnforce($starv['wref'], 3),
                $this->database->getEnforceVillage($starv['wref'], 2),
                $this->database->getEnforceVillage($starv['wref'], 3)
            ];
            
            $prisonerArrays = [
                $this->database->getPrisoners($starv['wref'], 1)
            ];
            
            $unitArrays = ($this->database->getUnitsNumber($starv['wref'], 0) > 0) ? [
                [
                    $this->database->getUnit($starv['wref'])
                ]
            ] : [];
            
            $attackArrays = [
                $this->database->getMovement(3, $starv['wref'], 0),
                $this->database->getMovement(4, $starv['wref'], 1)
            ];
            ...
            if (empty($starvingTroops))
                continue;
            
            // Counting
            $timedif = $time - $starv['starvupdate'];
            $cropProd = $this->database->getCropProdstarv($starv['wref']) - $starv['starv'];
            if ($cropProd < 0) {
                ...
                if ($difcrop > 0 && $oldcrop <= 0) {
                    $tribe = $this->database->getUserField(($type == 2) ? $starv['owner'] : $this->database->getVillageField($starvingTroops['from'], "owner"), "tribe", 0);
                    $start = ($special = in_array($type, [
                        1,
                        3
                    ])) ? 1 : ($tribe - 1) * 10 + 1;
                    ...
                    if ($starvingTroops[$heroType] > 0 && ($totalUnits == 0 || $totalUnits == $totalKilledUnits)) {
                        ...
                        $starvingTroops['heroinfo'] = $this->database->getHero(($type == 2) ? $starv['owner'] : $this->database->getVillageField(($type == 3 && $subtype == 1) ? $starvingTroops['to'] : $starvingTroops['from'], "owner"))[0];
                        $this->database->modifyHero("dead", 1, $starvingTroops['heroinfo']['heroid']);
                        $this->database->modifyHero("health", 0, $starvingTroops['heroinfo']['heroid']);
                        ...
                        switch ($type) {
                            case 0:
                                if ($totalKilledUnits < $totalUnits) {
                                    $this->database->modifyEnforce($starvingTroops['id'], array_keys($killedUnits), array_values($killedUnits), 0);
                                } else {
                                    $this->database->deleteReinf($starvingTroops['id']);
                                }
                                break;
                            
                            case 1:
                                if ($totalKilledUnits < $totalUnits) {
                                    $this->database->modifyPrisoners($starvingTroops['id'], array_keys($killedUnits), array_values($killedUnits), 0);
                                    $this->database->modifyUnit($starvingTroops['wref'], [
                                        "99o"
                                    ], [
                                        $totalKilledUnits
                                    ], [
                                    ...
                                    $this->database->deletePrisoners($starvingTroops['id']);
                                    $this->database->modifyUnit($starvingTroops['wref'], [
                                        "99o"
                                    ], [
                                        $totalUnits
                            ...
                            case 2:
                                $this->database->modifyUnit($starv['wref'], array_keys($killedUnits), array_values($killedUnits), [
                                    0
                                ]);
                                break;
                            ...
                            case 3:
                                if ($totalKilledUnits < $totalUnits) {
                                    $this->database->modifyAttack2($starvingTroops['id'], array_keys($killedUnits), array_values($killedUnits), 0);
                                } else {
                                    $this->database->setMovementProc($starvingTroops['moveid']);
                                }
                        }
                        
                        $this->database->modifyResource($starv['wref'], 0, 0, 0, max($newCrop, 0), 1);
                        $this->database->setVillageField($starv['wref'], [
                            'starv',
                            'starvupdate'
                        ], [
                            $upkeep,
            ...
            $crop = $this->database->getCropProdstarv($starv['wref'], false);
            if ($crop > $upkeep)
                $this->database->setVillageFields($starv['wref'], [
                    'starv',
                    'starvupdate'
                ], [
                    0,
    ...
    private function procNewClimbers()
    {
        ...
        if (count($climbers) > 0) {
            ...
            $result = mysqli_query($this->database->dblink, $q);
    ...
    private function procClimbers($uid)
    {
        ...
        if (count($this->ranking->getRank()) > 0 && $aid != 0) {
            ...
            $data = $this->database->getVSumField($memberIDs, "pop");
    ...
    private function regenerateOasisTroops()
    {
        ...
        if (count($array)) {
            ...
            foreach ($array as $oasis)
                $ids[] = $oasis['wref'];
            $this->database->regenerateOasisUnits($ids, true);
        }
    }
    ...
    function medals()
    {
        ...
        if ($giveMedal && MEDALINTERVAL > 0) {
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id, ap FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY ap DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                ...
                $resul = mysqli_query($this->database->dblink, $quer);
            }
            
            // Defender of the week
            $result = mysqli_query($this->database->dblink, "SELECT id, dp FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY dp DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "t3_" . ($i) . "";
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id, Rc FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY Rc DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "t1_" . ($i) . "";
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id, clp FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY clp DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "t6_" . ($i) . "";
            ...
            // Robbers of the week
            $result = mysqli_query($this->database->dblink, "SELECT id, RR FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY RR DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "t4_" . ($i) . "";
            ...
            // Top10 attackers
            $result = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY ap DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
                
                // Top 10 defenders
                $result2 = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY dp DESC, id DESC Limit 10");
                while ($row2 = mysqli_fetch_array($result2)) {
                    if ($row['id'] == $row2['id']) {
                        
                        $query3 = "SELECT Count(*) FROM " . TB_PREFIX . "medal WHERE userid=" . (int) $row['id'] . " AND categorie = 5";
                        $result3 = mysqli_query($this->database->dblink, $query3);
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                $result1 = mysqli_query($this->database->dblink, $query1);
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY dp DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY Rc DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY clp DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "users WHERE id > 5 AND access < 8 ORDER BY RR DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
            ...
            $result = mysqli_query($this->database->dblink, $query);
            $userIDs = [];
            ...
            if (count($userIDs)) {
                mysqli_query($this->database->dblink, "UPDATE " . TB_PREFIX . "users SET ap=0, dp=0,Rc=0,clp=0, RR=0 WHERE id IN(" . implode(', ', $userIDs) . ")");
            }
            
            // Start alliance Medals wooot
            
            // Aanvallers v/d Week
            $result = mysqli_query($this->database->dblink, "SELECT id, ap FROM " . TB_PREFIX . "alidata ORDER BY ap DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "a2_" . ($i) . "";
            ...
            // Verdediger v/d Week
            $result = mysqli_query($this->database->dblink, "SELECT id, dp FROM " . TB_PREFIX . "alidata ORDER BY dp DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "a3_" . ($i) . "";
            ...
            $result = mysqli_query($this->database->dblink, "SELECT id, RR FROM " . TB_PREFIX . "alidata ORDER BY RR DESC, id DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "a4_" . ($i) . "";
            ...
            // Rank climbers of the week
            $result = mysqli_query($this->database->dblink, "SELECT id, clp FROM " . TB_PREFIX . "alidata ORDER BY clp DESC Limit 10");
            $i = 0;
            while ($row = mysqli_fetch_array($result)) {
                $i++;
                $img = "a1_" . ($i) . "";
            ...
            $result = mysqli_query($this->database->dblink, "SELECT * FROM " . TB_PREFIX . "alidata ORDER BY ap DESC, id DESC Limit 10");
            while ($row = mysqli_fetch_array($result)) {
                
                // Pak de top10 verdedigers
                $result2 = mysqli_query($this->database->dblink, "SELECT id FROM " . TB_PREFIX . "alidata ORDER BY dp DESC, id DESC Limit 10");
                while ($row2 = mysqli_fetch_array($result2)) {
                    if ($row['id'] == $row2['id']) {
            ...
            if (count($aliIDs)) {
                mysqli_query($this->database->dblink, "UPDATE " . TB_PREFIX . "alidata SET ap=0, dp=0,RR=0,clp=0 WHERE id IN(" . implode(', ', $aliIDs) . ")");
            }
    ...
    private function artefactOfTheFool()
    {
        ...
        if ($array) {
            foreach ($array as $artefact) {
                ...
                mysqli_query($this->database->dblink, "UPDATE " . TB_PREFIX . "artefacts SET kind = " . (int) $kind . ", bad_effect = $bad_effect, effect2 = $effect, lastupdate = $time WHERE id = " . (int) $artefact['id']);
            }
        }
    }
}
