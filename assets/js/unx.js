...
function db() {
    return Math.round(nb() / 1000);
}

function ob(pb) {
    p = pb.innerHTML.split(":");
...
function rb(s, sb) {
    var tb, ub, vb;
    if (s > -2) {
        tb = Math.floor(s / 3600);
        ub = Math.floor(s / 60) % 60;
        vb = s % 60;
        t = tb + ":";
        if (ub < 10) {
            t += "0";
...
function fi2(qact, qact2, qact3) {
    ...
    return gi + '&cr=' + bi(4, ci) + '&qact=' + qact + '&qact2=' + qact2 + '&qact3=' + qact3;
}
...
function qst_next2(jf, act, act2, act3) {
    ...
    if (jf) {
        id = {
            'x': document.getElementById('qst_val_x').value,
            'y': document.getElementById('qst_val_y').value
        };
    } else {
        id = {
    ...
    fd(fi2(act, act2, act3), function(mf) {
        for (var qd in mf) {
            quest[qd] = mf[qd];
        }
    }, 'POST', id);
...
function initCounter() {
    ...
    pb = document.getElementsByClassName("timer");
...
function add_res(res, gc) {
    hc = fb['l' + (5 - gc)].value;
    ic = haendler * carry;
    fc[gc] = jc(fc[gc], hc, ic, carry);
    document.getElementById(res).value = fc[gc];
}

function upd_res(res, gc, kc) {
    hc = fb['l' + (5 - gc)].value;
    ic = haendler * carry;
    if (kc) {
        lc = hc;
    } else {
        lc = parseInt(document.getElementById(res).value);
    }
    if (isNaN(lc)) {
        lc = 0;
    }
    fc[gc] = jc(parseInt(lc), hc, ic, 0);
    document.getElementById(res).value = fc[gc];
}
...
function rc(n, d) {
    ...
    for (var i = 0; !x && d.layers && i < d.layers.length; i++) x = rc(n, d.layers[i].document);
    return x;
}
...
function uc() {
    ...
    $$('.popup3')[0].grab(new Element('div', {
        'id': 'drag'
    }), 'top').makeDraggable({
        'handle': 'drag'
    });
    if ($$('body')[0].getStyle('direction').toLowerCase() == 'rtl') {
        $$('.popup3')[0].setStyle('direction', 'rtl').getParent().setStyle('direction', 'ltr');
    }
}
...
function xy() {
    xc = screen.width + ":" + screen.height;
    document.snd.w.value = xc;
}
...
function my_village() {
    ...
    for (var i = 0; i < dorfnamen.length; i++) {
        if (dorfnamen[i].indexOf(e) > -1) {
            yc++;
            zc = dorfnamen[i];
        }
    }
    if (yc == 1) {
        document.snd.dname.value = zc;
    }
}
var $c = document.getElementById ? 1 : 0;
var _c = document.all ? 1 : 0;
var ad = (navigator.userAgent.indexOf("Mac") > -1) ? 1 : 0;
var bd = (_c && (!ad) && (typeof(window.offscreenBuffering) != 'undefined')) ? 1 : 0;
var cd = bd;
var dd = bd && (window.navigator.userAgent.indexOf("SV1") != -1);

function changeOpacity(ed, opacity) {
    if (bd) {
        ed.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + (opacity * 100) + ')';
    } else if ($c) {
        ed.style.MozOpacity = opacity;
    }
}
...
function fd(url, gd, hd, id) {
    ...
    if (window.XMLHttpRequest) {
        ...
        try {
            jd = new ActiveXObject("Msxml2.XMLHTTP");
        } catch (e) {
            try {
                jd = new ActiveXObject("Microsoft.XMLHTTP");
            } catch (e) {}
        }
    } else {
...
function md(nd) {
    ...
    var pd = true;
    for (var qd in nd) {
        od += (pd ? '' : '&') + qd + '=' + window.encodeURI(nd[qd]);
        if (pd) {
            pd = false;
        }
    }
...
function map_init() {
    ...
    ke = ['mcx', 'mcy'];
    for (var i = 0; i < ke.length; i++) {
        ee[ke[i]].onfocus = function() {
            vd = true;
        };
        ee[ke[i]].onblur = function() {
            vd = false;
        };
    }
...
function $e(_e) {
    var af;
    var bf;
    if (ee.map_infobox != null) {
        if (_e.normal_field) {
            var cf = df(_e.nr * 1);
            af = [text_k.verlassenes_tal + ': ' + cf.join('-')];
            bf = 'empty';
        } else if (_e.free_oasis && !_e.classic_oasis) {
            af = [text_k.freie_oase];
            bf = 'oasis_empty';
        } else if (_e.occupied_oasis && !_e.classic_oasis) {
            af = [text_k.besetztes_tal, _e.name, _e.ew, _e.ally];
            bf = 'oasis';
        } else if (_e.village) {
            var text = '<span class="tribe tribe' + _e.vid + '">' + _e.dname + '</span>';
            af = [text, _e.name, _e.ew, _e.ally];
            bf = 'village';
        } else {
            af = [text_k.details];
            bf = 'default';
    ...
    for (var i = 0; i < 4; i++) {
        ee.ibox_cells[i].innerHTML = (af[i] === undefined || af[i] === '') ? '-' : af[i].replace(/\&/g, "&amp;");
    }
...
function hf(jf) {
    ee.x.firstChild.nodeValue = jf.x;
    ee.y.firstChild.nodeValue = jf.y;
}

function kf(jf) {
    ee.mcx.value = jf.x;
    ee.mcy.value = jf.y;
    l = $$('div.cropfinder_icon a').set('href', 'cropfinder.php?x=' + jf.x + '&y=' + jf.y);
}
...
function ve() {
    if (mmode) {
        zd = window.open(this.href, "map", "top=100,left=25,width=1007,height=585");
        zd.focus();
    } else {
        xe(true);
        fd('ajax.php?f=kp&z=' + lf(m_c.z.x, m_c.z.y), function(mf) {
            xe(false);
            ae = document.getElementsByTagName('body')[0];
            be = document.getElementsByTagName('html')[0];
            ae.parentNode.removeChild(ae);
            ce = document.createElement('body');
            ce.innerHTML = mf.lm;
            ce.style.backgroundColor = '#ffffff';
            be.appendChild(ce);
            m_c = [];
            for (var qd in mf.dat.m_c) {
                m_c[qd] = mf.dat.m_c[qd];
            }
            m_c.fields = [];
            mdim = mf.dat.mdim;
            mmode = mf.dat.mmode;
...
function we() {
    if (fe) {
        pb = parent.document.getElementById("drag2");
        if (pb != null) {
            pb.innerHTML = ''
        } else {
            window.close();
        ...
        for (var i = 0; i < mdim.x; i++) {
            for (var j = 0; j < mdim.y; j++) {
                area = pe(i, j, 'a'); of = pe(i, j, 'i');
                area.details = nf[i][j];
                area.details.fresh = {}; of .className = nf[i][j].img;
                pf(area, of );
            }
        }
        hf(m_c.z);
...
function bg(cg) {
    return 'ajax.php?f=k7&x=' + cg.x + '&y=' + cg.y + '&xx=' + cg.xx + '&yy=' + cg.yy;
}
...
function sg() {
    m_c.index++;
    if (m_c.index == m_c.size - 2) {
        m_c.usealternate = true;
    }
}

function lg() {
    m_c.cindex = pg(m_c.cindex);
}

function mg(qf, rf, dg) {
    var tg = document.getElementById('map_content');
    var ug = tg.parentNode;
    if (1 == rf) {
        vg(qf);
        wg(m_c.fields[m_c.cindex], qf, rf);
        qg(qf);
        sg();
    } else if (mdim.x == rf || dg !== undefined) {
        xg(m_c.fields[m_c.cindex]);
    }
    if (wd == 0) {
        kf(m_c.z);
    }
...
function yg(ah, bh, _e) {
    var of = pe(ah, bh, 'i');
    var area = pe(ah, bh, 'a');
    te(_e, area); of .className = area.details.img;
    pf(area, of );
}

function pf(area, of ) {
    if (area.details.atyp) {
        if (! of .firstChild) { of .appendChild(document.createElement('span'));
        } of .firstChild.className = 'm' + area.details.atyp;
    } else {
        if ( of .firstChild) { of .removeChild( of .firstChild);
        }
    }
}
...
function te(_e, area) {
    area.details = {};
    var ch = ['x', 'y', 'nr', 'typ', 'querystring', 'img', 'atyp', 'dname', 'name', 'ew', 'ally', 'vid', 'atime'];
    for (var i = 0; i < _e.length; i++) {
        area.details[ch[i]] = _e[i];
    }
    area.details.normal_field = area.details.name === undefined && area.details.typ == 0;
    area.details.free_oasis = area.details.name === undefined && area.details.typ != 0;
    area.details.occupied_oasis = area.details.name !== undefined && area.details.typ != 0;
    area.details.village = area.details.name !== undefined && area.details.typ == 0;
    area.details.fresh = {};
...
function fh(area) {
    if (!area.details.fresh.href) {
        if (area.details.classic_oasis) {
            ...
            area.href = 'karte.php?' + area.details.querystring;
...
function gh(area) {
    if (area.details.normal_field) {
        area.title = '';
    } else if (area.details.free_oasis && !area.details.classic_oasis) {
        area.title = text_k.freie_oase;
    } else if (area.details.occupied_oasis && !area.details.classic_oasis) {
        area.title = text_k.besetztes_tal;
    } else if (area.details.village) {
        if (area.details.atime !== undefined) {
            area.title = area.details.dname + ' ' + rb(area.details.atime - Math.floor(new Date().getTime() / 1000), true);
            return false;
        } else {
            area.title = area.details.dname;
        }
    } else {
        area.title = '';
    }
    ...
    for (var i = 0; i < mdim.x; i++) {
        switch (qf) {
            ...
            case 3:
                ih = i;
                jh = 0;
                _e = ng[i][m_c.size - m_c.index - 1];
                break;
            case 4:
                ih = 0;
                jh = i;
                _e = ng[m_c.size - m_c.index - 1][i];
                break;
        }
        yg(ih, jh, _e);
    }
...
function kh(x, y, tf, uf) {
    ...
    if ( of .firstChild) {
        if (!lh.firstChild) {
            lh.appendChild(document.createElement('span'))
        }
        lh.firstChild.className = of .firstChild.className;
    } else {
        if (lh.firstChild) {
            lh.removeChild(lh.firstChild);
        }
    }
    nh(x, y, tf, uf);
}

function nh(x, y, tf, uf) {
    _e = pe(x, y, 'a');
    oh = pe(tf, uf, 'a');
    oh.details = _e.details;
    oh.details.fresh = {};
}

function mh( of , lh) {
    lh.className = of .className;
}
        ...
        for (var j = 1; j < mdim.x; j++) {
            switch (qf) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    kh(i, mdim.x - 1 - j, i, mdim.x - j);
                    break;
                case 4:
                    kh(mdim.x - 1 - j, i, mdim.x - j, i);
                    break;
            }
        }
    }
...
function qg(rh) {
    ...
    for (var i = 0; i < mdim.x; i++) {
        ...
        if (ee[sh][i]) {
            th = pe(vh, wh, 'a').details[jb];
            ee[sh][i].firstChild.nodeValue = th;
        }
    }
}
//document.onkeydown=me;
...
function ie(zh) {
    ...
    for (var i = 0; i < zh.length; i++) {
        yh = zh[i];
        xh[yh] = [];
        for (var ah = 0; ah < mdim.x; ah++) {
            xh[yh][ah] = [];
            for (var bh = 0; bh < mdim.y; bh++) {
                xh[yh][ah][bh] = document.getElementById(yh + '_' + ah + '_' + bh);
            }
        }
    }
}
...
function bi(length, ci) {
    ...
    for (var i = 0; i < length; i++) {
        var ei = Math.floor((Math.random() + ci) * 0.5 * di.length);
        bi += di.substring(ei, ei + 1);
    }
    return bi;
}
...
function fi(qact, qact2) {
    ...
    var gi = 'ajax.php?f=qst';
    var ci = (Math.abs(quest.number) + 1) / (Math.abs(quest.last) + 1);
    return gi + '&cr=' + bi(4, ci) + '&qact=' + qact + '&qact2=' + qact2;
}
...
function hi() {
    ...
    for (var ji in step.target) {
        ...
        step.current[ji] = step.source[ji];
        step.step[ji] = Math.round((step.target[ji] - step.source[ji]) / step.n);
    }
    step.timeout = 1000 / step.fps;
    quest.cstep = step;
    quest.anmlock = true;
...
function ii(mi) {
    ...
    if (mi) {
        var oi = '<div class="popup3 quest"><a href="#" onClick="qst_handle()"><img src="img/x.gif" border="1" class="popup4" alt="Close" title="Close"></a><div id="popup3"</div></div>';
        ni.innerHTML = oi;
        pi();
        qst_wfm();
        vc();
    ...
    if (step.i < step.n) {
        ...
        if (quest.anmstep === false) {
            ...
            if (quest.number >= quest.last || quest.altstep == 9) {
                document.getElementById('qge').innerHTML = '';
            }
        }
    }
}
        ...
        if (quest.reward.finish && window.bld) {
            ...
            if (xi !== false) {
                ...
                if (zi) {
                    ...
                    $$('.f3')[0].appendChild(new Element('img', {
                        'class': ('reslevel rf' + bld[xi].aid + ' level' + bld[xi].stufe),
                        'src': 'img/x.gif'
                    }));
                }
            }
...
function pi() {
    document.getElementById('popup3').innerHTML = '<img src="img/x.gif" class="xlo" />';
}

function qst_enter(jf) {
    if (jf === undefined) {
        jf = false;
    }
    var id;
...
function wi(_i) {
    var $i = document.getElementById('qgei');
    if ($i && _i.qgsrc) {
        $i.className = _i.qgsrc;
    }
    var aj = document.getElementById('n5');
    if (aj && _i.msrc) {
        aj.className = _i.msrc;
    }
    if (_i.cookie) {
        var date = new Date();
        date.setTime(date.getTime() + 300000);
        document.cookie = 't3fw=1; expires=' + date.toUTCString() + ';';
    }
    if (_i.fest && he('dorf2')) {
        document.getElementById('content').innerHTML += _i.fest;
    }
    window.setTimeout(ti, 30);
}

function he(bj) {
    return window.location.href.indexOf(bj + '.php') != -1;
}
...
function vil_levels_toggle() {
    ...
    dj.toggleClass('on');
    if (cj.hasClass('on')) {
        document.cookie = 'SHOWLEVELS=1; expires=Wed, 1 Jan 2020 00:00:00 GMT';
    } else {
        document.cookie = 'SHOWLEVELS=1; expires=Thu, 01-Jan-1970 00:00:01 GMT';
    }
}

function gmwd() {
    if (is_ff2 && document.getElementById("gmwi").offsetWidth < 50) {
        document.cookie = "a3=2; expires=Wed, 1 Jan 2020 00:00:00 GMT";
    } else {
        document.cookie = "a3=1; expires=Wed, 1 Jan 2020 00:00:00 GMT";
    }
}

function gmc() {
    document.getElementById("gmw").style.display = "none";
    document.cookie = "a3=3; expires=Wed, 1 Jan 2020 00:00:00 GMT";
}
